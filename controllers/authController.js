        ...
        try {
            ...
            bcrypt.genSalt(10, (err, salt) => {
                ...
                bcrypt.hash(password, salt, async (err, hash) => {
                    ...
                    jwt.sign({ userId: user.id }, config.jwtSecret, { expiresIn: 600 * 60 }, (err, token) => {
                        if (err) throw err;
                        res.json({
                            token,
                            email
    ...
    async login(req, res) {
        ...
        try {
            ...
            if (!!user) {
                ...
                if (!role) return res.status(403).json({ message: "No role assign to this user !" })
                permissions = await PermissionModel.find({'role_id': role._id })
            }
