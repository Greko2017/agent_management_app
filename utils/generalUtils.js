...
module.exports = {
    ...
    generateNameServerSide(seq){
        const today = new Date()
        return `${seq}_${today.getFullYear()}${today.getMonth()}${today.getDate()}${today.getMilliseconds()}`
    },

    async sendCredentialEmail(data, email){  
        // console.log('in sendCredentialEmail', data,email )
    ...
    var transporter = nodemailer.createTransport({
      ...
      secure: true, // use SSL
      auth: {
          user: process.env.MAIL_FROM || "gregory.goufan@gmail.com",
          pass: process.env.GOOGLE_APP_KEY || "gsmuyhlybstkllwo"
      }
  });
  
    var mailOptions = {
        from: '"ATE" <info@eta-platform.com>', // sender address (who sends)
        to: `${email}`, // list of receivers (who receives) 
        subject: 'ETA Platform | Credential', // Subject line
        text: 'Info Client!', // plaintext body
        html: sendCredentialEmailTemplate({message:message}), // html body
    };
